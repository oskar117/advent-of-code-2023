...
class Part2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("New total winnings are " + result);
    }
    ...
    static class Hand implements Comparable<Hand> {
        ...
        int calculatePower() {
            ...
            Map.Entry<Card, Long> max = Collections.max(collect.entrySet(), (a, b) -> a.getKey().symbol.equals("J") ? -1 : a.getValue().compareTo(b.getValue()));
            Long jokerCount = Optional.ofNullable(collect.get(new Card("J"))).orElse(0L);
            if (!Objects.equals(max.getKey().symbol, "J")) {
                max.setValue(max.getValue() + jokerCount);
            }
            if (max.getValue() == 5) return 21;
            else if (max.getValue() == 4) return 20;
            else if ((collect.size() == 2 || (collect.size() == 3 && jokerCount > 0)) && max.getValue() == 3) return 19;
            else if ((collect.size() == 3 || (collect.size() == 4 && jokerCount > 0)) && max.getValue() == 3) return 18;
            else if ((collect.size() == 3 || (collect.size() == 4 && jokerCount > 0)) && max.getValue() == 2) return 17;
            else if ((collect.size() == 4 || (collect.size() == 5 && jokerCount > 0)) && max.getValue() == 2) return 16;
            else return 15;
        }
